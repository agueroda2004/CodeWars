...
export const stockList = (listOfArt: string[], listOfCat: string[]) => {
  ...
  for (let i = 0; i < listOfArt.length; i++) {
    let cate: string = listOfArt[i][0];
    let stock: number = Number(listOfArt[i].split(" ")[1]);

    result.set(cate, (result.get(cate) ?? 0) + stock);
  }

  return listOfCat.map((c) => `(${c} : ${result.get(c) ?? 0})`).join(" - ");
};
